...
func NewClient(input *NewClientInput) *Client {
	publicURL := url.URL{
		Scheme: ifThenElse(input.PublicSiteUseTLS, "https", "http").(string),
		Host:   input.PublicSite,
	}
	...
	baseURL := url.URL{
		Scheme: ifThenElse(input.SiteUseTLS, "https", "http").(string),
		Host:   input.Site,
	}
...
// do use the client to send a specified request
